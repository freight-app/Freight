...
const HEADER: &str =
    "# This file is automatically @generated by freight.\n# Do not edit manually.\n";
...
impl LockFile {
    ...
    pub fn upsert_registry_dep(
        ...
        if let Some(pkg) = lock.packages.iter_mut().find(|p| p.name == name) {
            ...
            pkg.checksum = if checksum.is_empty() {
                None
            } else {
    ...
    pub fn save(&self, project_dir: &Path) -> std::io::Result<()> {
        std::fs::write(project_dir.join("freight.lock"), self.render())
    }
...
fn dep_checksum(dep_dir: &Path) -> Option<String> {
    let content = std::fs::read(dep_dir.join("freight.toml")).ok()?;
