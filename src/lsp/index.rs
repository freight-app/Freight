...
/// Token-type names emitted by freight's native indexers (clang-bridge, Fortran,
/// asm). The index of each name is the `token_type` value those indexers emit
/// (0 = namespace … 8 = macro). When freight advertises a *forwarded* server's
...
pub fn semantic_tokens_legend() -> Value {
    serde_json::json!({
        "tokenTypes": FREIGHT_TOKEN_TYPES,
        "tokenModifiers": []
    })
}
...
pub enum HeaderOrigin {
    ...
    Fetched,
    /// Produced by a build plugin's codegen into `target/<profile>/plugin-gen/…`.
    /// `package_name` carries the plugin that generated it.
    Generated,
...
/// A plugin-generated output directory and the plugin that owns it. Headers
/// found here are indexed with [`HeaderOrigin::Generated`] so hover/inlay hints
/// can say "generated by …" instead of treating them as project-owned files.
pub struct GeneratedDirSpec<'a> {
    /// The `OUT_DIR` to walk for generated headers.
    pub path: &'a Path,
    /// The plugin package that produces the directory's contents.
...
pub fn build_source_indexes(
    ...
    generated_dirs: &[GeneratedDirSpec<'_>],
    sysroot: Option<&Path>,
    target: Option<&str>,
) -> SourceIndexes {
    ...
    if let Some(pkgs) = pkgs_dir.filter(|p| p.is_dir()) {
        for entry in std::fs::read_dir(pkgs).into_iter().flatten().flatten() {
            ...
            let dir_name = pkg_dir
                ...
                .unwrap_or("")
                .to_string();
            // The `.pkgs/` directory is named just `<name>` (no version), so the
            // version comes from the fetched package's own freight.toml. Foreign
            // deps without a manifest fall back to the directory name and have no
            // known version.
            let manifest = load_manifest_cached(&pkg_dir).ok();
            let pkg_name = manifest
                .as_ref()
                .map(|m| m.package.name.clone())
                .filter(|s| !s.is_empty())
    ...
    // Plugin-generated output dirs (target/<profile>/plugin-gen/<section>).
    // Headers here are first-party (they resolve like project headers) but are
    // labelled as generated so hover/inlay hints credit the plugin.
    for spec in generated_dirs {
        if !spec.path.is_dir() {
            continue;
        }
        walk_headers(
...
pub fn package_tooltip(
    ...
    if matches!(origin, HeaderOrigin::Generated) {
        return format!("**{fallback_name}** — generated by build plugin");
    }
...
pub fn include_hint_line(header: &str, entry: &HeaderEntry) -> String {
    ...
    if entry.origin == HeaderOrigin::Generated {
        return format!("**{pkg}** (generated)/{file}");
    }
...
/// Short label for an inlay hint. The version is kept for the tooltip; the
/// inline label stays terse:
/// - system headers → `← stdlib`
/// - a **local** project header → its directory (relative to the package root),
///   e.g. `← include/geometry` — more useful than repeating the project's name
/// - a **dependency** header → the dependency it comes from, e.g. `← zlib`
pub fn include_inlay_label(entry: &HeaderEntry) -> String {
    match entry.origin {
        HeaderOrigin::System => "← stdlib".to_string(),
        HeaderOrigin::Generated => format!("← generated ({})", entry.package_name),
        HeaderOrigin::Own => match local_header_dir(entry) {
            Some(dir) => format!("← {dir}"),
            None => format!("← {}", entry.package_name),
        },
        _ => format!("← {}", entry.package_name),
    }
}

/// Directory of a local (`Own`) header relative to its package root, with
/// forward slashes. `None` when it can't be derived or the header sits directly
/// at the package root.
fn local_header_dir(entry: &HeaderEntry) -> Option<String> {
    let parent = entry.full_path.parent()?;
    let rel = match &entry.pkg_dir {
        ...
        IncludeCompletionCtx::Angled(_) | IncludeCompletionCtx::Quoted(_) => {
            ...
            for (path, entry) in index.completion_entries() {
                let detail = match &entry.origin {
                    ...
                    HeaderOrigin::Own => "this project".to_string(),
                    HeaderOrigin::Generated => format!("generated by {}", entry.package_name),
...
mod tests {
    ...
    #[test]
    fn include_label_shows_dir_for_local_and_dep_name_for_dependency() {
        // Local project header → its directory relative to the package root.
        let own = HeaderEntry {
            package_name: "app".into(),
            package_version: None,
            full_path: PathBuf::from("/proj/include/geometry/vec.h"),
            origin: HeaderOrigin::Own,
            dep_key: None,
            pkg_dir: Some(PathBuf::from("/proj")),
        };
        assert_eq!(include_inlay_label(&own), "← include/geometry");

        // A local header at the package root has no subdir → fall back to name.
        let root = HeaderEntry {
            full_path: PathBuf::from("/proj/config.h"),
            pkg_dir: Some(PathBuf::from("/proj")),
            ..own.clone()
        };
        assert_eq!(include_inlay_label(&root), "← app");

        // Dependency header → the dependency name, not its directory.
        let dep = HeaderEntry {
            package_name: "zlib".into(),
            package_version: Some("1.3".into()),
            full_path: PathBuf::from("/proj/.pkgs/zlib/include/zlib.h"),
            origin: HeaderOrigin::Fetched,
            dep_key: Some("zlib".into()),
            pkg_dir: Some(PathBuf::from("/proj/.pkgs/zlib")),
        };
        assert_eq!(include_inlay_label(&dep), "← zlib");
        ...
        assert_eq!(include_inlay_label(&sys), "← stdlib");
    }
    ...
    #[test]
    fn fetched_pkg_version_comes_from_its_manifest() {
        // `.pkgs/<name>` is named without a version, so the version must be read
        // from the fetched package's own freight.toml.
        let tmp = tempfile::tempdir().unwrap();
        let pkgs = tmp.path().join(".pkgs");
        let zlib = pkgs.join("zlib"); // note: no `-1.3.2` suffix
        std::fs::create_dir_all(zlib.join("include")).unwrap();
        std::fs::write(
            zlib.join("freight.toml"),
            "[package]\nname = \"zlib\"\nversion = \"1.3.2\"\n",
        )
        .unwrap();
        std::fs::write(zlib.join("include/zlib.h"), "// header").unwrap();

        let index = build_source_indexes(&[], Some(&pkgs), &[], None, None).headers;
        let entry = index.lookup("zlib.h").expect("zlib.h indexed");
        assert_eq!(entry.package_name, "zlib");
        assert_eq!(entry.package_version.as_deref(), Some("1.3.2"));
        assert_eq!(entry.origin, HeaderOrigin::Fetched);
        // And the rendered hint carries the version.
        assert_eq!(include_hint_line("zlib.h", entry), "**zlib@1.3.2**/zlib.h");
    }
    ...
    #[test]
    fn generated_headers_are_indexed_with_plugin_provenance() {
        let tmp = tempfile::tempdir().unwrap();
        // Simulate a plugin OUT_DIR holding a generated header.
        let out = tmp.path().join("target/debug/plugin-gen/proto");
        std::fs::create_dir_all(&out).unwrap();
        std::fs::write(out.join("foo.pb.h"), "// generated").unwrap();

        let gen = [GeneratedDirSpec {
            path: out.as_path(),
            plugin_name: "proto",
        }];
        let index = build_source_indexes(&[], None, &gen, None, None).headers;

        let entry = index.lookup("foo.pb.h").expect("generated header indexed");
        assert_eq!(entry.origin, HeaderOrigin::Generated);
        assert_eq!(entry.package_name, "proto");
        // Labels credit the plugin rather than treating it as project-owned.
        assert_eq!(include_inlay_label(entry), "← generated (proto)");
        assert_eq!(
            include_hint_line("foo.pb.h", entry),
            "**proto** (generated)/foo.pb.h"
        );
        assert_eq!(
            package_tooltip(
            ...
            "**proto** — generated by build plugin"
        );
    }
