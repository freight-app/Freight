...
//! Copied from docify `util/latex.rs`; `protect_math` inlined from `markdown.rs`.

// ── Math region detection (from docify markdown.rs) ──────────────────────────
...
fn protect_math(text: &str) -> (String, Vec<MathRegion>) {
