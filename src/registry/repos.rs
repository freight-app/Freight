...
pub fn repo_by_name(
    ...
    match name {
        ...
        // The local system-side registry generated by `freight-system-registry`.
        "system" => {
            return super::DirectoryRegistry::system()
                .map(|r| -> Box<dyn PackageRepo> { Box::new(r) })
                .ok_or_else(|| {
                    FreightError::RegistryError(
                        "no system registry; set FREIGHT_HOME and run `freight-system-registry`"
                            .into(),
                    )
                });
        }
