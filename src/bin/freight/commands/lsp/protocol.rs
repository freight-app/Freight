...
pub fn read_lsp_message<R: BufRead>(reader: &mut R) -> io::Result<Option<Value>> {
    ...
    let Some(len) = content_len else { return Ok(None); };
...
pub fn sanitize_code_action_diagnostics(msg: &Value) -> Value {
    ...
    for diagnostic in diagnostics {
        let Some(obj) = diagnostic.as_object_mut() else { continue; };
        let Some(code) = obj.get("code").cloned() else { continue; };
        if code.is_string() { continue; }
        let replacement = code.as_i64().map(|n| n.to_string())
...
pub fn merge_capability_object(into: &mut Value, from: &Value) {
    let Some(into_obj) = into.as_object_mut() else { *into = from.clone(); return; };
    let Some(from_obj) = from.as_object() else { return; };
...
fn merge_completion_provider(into_obj: &mut serde_json::Map<String, Value>, from: &Value) {
    let entry = into_obj.entry("completionProvider".to_string()).or_insert_with(|| json!({}));
    ...
    if let Some(from_obj) = from.as_object() {
        for (key, value) in from_obj {
            if key == "triggerCharacters" {
                let mut triggers = entry_obj.get("triggerCharacters")
                    .and_then(Value::as_array).cloned().unwrap_or_default();
                for item in value.as_array().into_iter().flatten() {
                    if !triggers.iter().any(|e| e == item) { triggers.push(item.clone()); }
                }
...
fn merge_signature_help_provider(into_obj: &mut serde_json::Map<String, Value>, from: &Value) {
    let entry = into_obj.entry("signatureHelpProvider".to_string()).or_insert_with(|| json!({}));
    if !entry.is_object() { *entry = json!({}); }
    let Some(entry_obj) = entry.as_object_mut() else { return; };
    if let Some(from_obj) = from.as_object() {
        for key in ["triggerCharacters", "retriggerCharacters"] {
            if let Some(value) = from_obj.get(key) {
                let mut chars = entry_obj.get(key).and_then(Value::as_array)
                    .cloned().unwrap_or_default();
                for item in value.as_array().into_iter().flatten() {
                    if !chars.iter().any(|e| e == item) { chars.push(item.clone()); }
                }
...
pub fn root_from_initialize(msg: &Value) -> Option<PathBuf> {
    ...
    params.get("rootUri").and_then(Value::as_str).and_then(path_from_uri)
...
pub fn changed_full_text(msg: &Value) -> Option<String> {
    msg.get("params")?.get("contentChanges")?.as_array()?
        .last()?.get("text")?.as_str().map(ToString::to_string)
}

pub fn text_document_uri(msg: &Value) -> Option<String> {
    msg.get("params")?.get("textDocument")?.get("uri")?
