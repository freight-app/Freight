//! `crane debug` — build with debug symbols and launch an interactive debugger,
//! or generate IDE launch configurations via `--launch-json`.

use std::path::{Path, PathBuf};

use crane_core::build::{build_project, BuildOutput};
use crane_core::manifest::{find_manifest_dir, load_manifest};
use crane_core::toolchain::{detect_debuggers, load_debugger_templates};
...
pub fn cmd_debug(
    ...
    let output = match build_project("debug") {
        Ok(o) => o,
        Err(e) => { print_error(&e.to_string()); return; }
    };
    ...
    // ── Exec the debugger (replace the crane process on Unix) ──────────────────
...
fn gen_launch_json(
    project_dir: &Path,
    manifest: &crane_core::manifest::types::Manifest,
    debuggers: &[crane_core::toolchain::DetectedDebugger],
) {
    ...
    let mut kept: Vec<serde_json::Value> = existing
        ...
        .map(|arr| {
            arr.iter()
                .filter(|c| c.get("generatedBy").and_then(|v| v.as_str()) != Some("crane"))
                .cloned()
                .collect()
        })
        .unwrap_or_default();
    ...
    for bin in &manifest.bins {
        ...
        for dbg in debuggers {
            let label = if debuggers.len() == 1 {
                format!("Debug {} (crane)", bin.name)
            } else {
                format!("Debug {} ({}, crane)", bin.name, dbg.template.name)
            };
...
fn select_binary(
    ...
    match candidates.len() {
        ...
        _ => Err(format!(
            "multiple binaries: {}; specify one with `crane debug <name>`",
            candidates.iter()
...
/// On Unix: exec() replaces the crane process so the terminal connects
