//! Hover documentation for `freight.toml` fields.

/// Return Markdown documentation for a dotted path into `freight.toml`,
    ...
    match path {
        ...
        "dependencies" => Some(
            "Runtime dependencies. Each entry is one of:\n\n\
             - **Version**: `foo = \"0.3\"` — fetched from freight.dev (not yet implemented)\n\
             - **Path**: `foo = { path = \"../foo\" }` — sibling freight project, built + linked\n\
