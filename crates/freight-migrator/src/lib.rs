//! Migrate an existing CMake / Makefile / Meson project into a `freight.toml`.
//!
...
//! freight manifest. Constructs the importer could not translate are preserved
//! as `# FREIGHT: …` comments at the top of the emitted file so the user can
//! review them.
...
pub struct MigrateOutcome {
    ...
    /// The root `freight.toml` path (workspace root or single project), or
    /// `None` for `--dry-run`.
    pub written_to: Option<PathBuf>,
    /// For workspace migrations: `(relative_path, toml_content)` for each
    /// member freight.toml. Empty for single-project migrations.
    pub workspace_members: Vec<(String, String)>,
    /// Generated root `freight.toml` contents — useful for `--dry-run` output.
    pub toml: String,
    /// Number of `# FREIGHT:` notes the user should review (across all files).
...
pub fn run_migrate(
    ...
    dry_run: bool,
    force: bool,
) -> Result<MigrateOutcome, FreightError> {
    let fmt = match format {
        Some(f) => f,
        None => detect::detect_format(project_dir)
            .ok_or_else(|| FreightError::ImporterNoFormat(project_dir.display().to_string()))?,
    };

    let manifest_path = project_dir.join("freight.toml");
    if !dry_run && manifest_path.exists() && !force {
        return Err(FreightError::ImporterManifestExists(
...
pub fn parse_format(s: &str) -> Result<Format, FreightError> {
    Format::from_str(s)
}
...
fn run_migrate_workspace(
    ...
    let written_to = if dry_run {
        ...
        // Write workspace root freight.toml.
        fs::write(project_dir.join("freight.toml"), &ws.root_toml)?;

        // Write each member's freight.toml, creating the subdirectory as needed.
        for (rel, content) in &ws.members {
            let member_dir = project_dir.join(rel);
            fs::create_dir_all(&member_dir)?;
            fs::write(member_dir.join("freight.toml"), content)?;
        }

        Some(project_dir.join("freight.toml"))
    };
...
pub struct ImportedProject {
    ...
    /// Keyed by freight language identifier: `"c"`, `"cpp"`, `"fortran"`, etc.
    ...
    /// Per-platform overlays keyed by freight platform name (`linux`, `windows`,
    ...
    /// `[platform.X]` sections in the generated `freight.toml`.
    pub platforms: BTreeMap<String, ImportedPlatformOverlay>,
    /// Free-form notes emitted as `# FREIGHT: …` comments at the top of the
...
pub struct ImportedBin {
    ...
    /// discovered by freight's build engine at compile time.
    pub src: String,
}
...
pub enum ImportedDep {
    ...
    System(String),
    /// A bare version string — resolved against freight.dev once available.
    Version(String),
    /// `{ path = "../foo" }` — sibling freight project.
    Path(String),
}
