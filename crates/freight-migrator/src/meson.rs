...
//! records anything else it doesn't recognise as a `# FREIGHT:` note.
...
pub fn parse(project_dir: &Path) -> Result<ImportedProject, FreightError> {
    let path = project_dir.join("meson.build");
    let text = fs::read_to_string(&path)
        .map_err(|e| FreightError::ImporterParse(format!("reading {}: {e}", path.display())))?;
        ...
        match c.name.as_str() {
            ...
            "test" => {
                if let Some(name) = c.positional.first() {
                    project.push_note(format!(
                        "test('{name}'): test target not imported — add to freight tests manually"
                    ));
                }
            }
            "configure_file" => {
                project.push_note(
                    "configure_file(): not imported — recreate in a build.freight script"
                        .to_string(),
                );
            }
            "custom_target" => {
                if let Some(name) = c.positional.first() {
                    project.push_note(format!(
                        "custom_target('{name}'): not imported — recreate in a build.freight script"
                    ));
                }
            }
            _ => {}
