...
//! pulls out the pieces that map cleanly to freight (compiler variables, flag
...
pub fn parse(project_dir: &Path) -> Result<ImportedProject, FreightError> {
    let path = if project_dir.join("Makefile").is_file() {
        ...
        return Err(FreightError::ImporterParse(format!(
...
fn extract_flags(project: &mut ImportedProject, text: &str) {
    ...
    while i < tokens.len() {
        ...
        if let Some(rest) = tok.strip_prefix("-D") {
            ...
            // Represented via freight's structured profile/warnings fields.
