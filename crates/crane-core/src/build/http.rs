...
pub fn fetch_url_dep(
    ...
    project_dir: &Path,
) -> Result<PathBuf, CraneError> {
    let deps_dir = project_dir.join(".deps").join(name);
    let sentinel = deps_dir.join(".crane-fetched");
    ...
    let status = Command::new("curl")
        ...
        .arg(url)
        .status()
        .map_err(|e| CraneError::CompilerNotFound(format!("curl not found: {e}")))?;
...
pub fn pkg_config_query(query: &str) -> Result<PkgConfigResult, CraneError> {
...
fn run_pkg_config(query: &str, flag: &str) -> Result<String, CraneError> {
    let parts: Vec<&str> = query.split_whitespace().collect();
    let out = Command::new("pkg-config")
        .arg(flag)
        .args(&parts)
        .output()
        .map_err(|e| CraneError::CompilerNotFound(format!("pkg-config not found: {e}")))?;
...
fn sha256_of_file(path: &Path) -> Result<String, CraneError> {
...
fn extract_archive(archive: &Path, dest: &Path, ext: &str) -> Result<(), CraneError> {
    ...
    let ok = if ext == "zip" {
        Command::new("unzip")
            ...
            .status()
            .map_err(|e| CraneError::CompilerNotFound(format!("unzip not found: {e}")))?
            .success()
    } else {
        ...
        Command::new("tar")
            ...
            .status()
            .map_err(|e| CraneError::CompilerNotFound(format!("tar not found: {e}")))?
            .success()
    };
