...
/// contains `crane.toml`, or `None` if none is found.
pub fn find_manifest_dir(start: &Path) -> Option<PathBuf> {
    let mut current = start.canonicalize().ok()?;
    loop {
        if current.join("crane.toml").is_file() {
            return Some(current);
        }
        if !current.pop() {
            return None;
...
mod tests {
    ...
    fn finds_manifest_in_same_dir() {
        ...
        fs::write(tmp.path().join("crane.toml"), "[package]\nname = \"x\"\n").unwrap();
    ...
    fn returns_none_when_no_manifest() {
        ...
        // We can't guarantee nothing above tmp has crane.toml, so instead
        // test that the function at least returns Some when we add one.
        fs::write(tmp.path().join("crane.toml"), "").unwrap();
    ...
    fn prefers_nearest_manifest() {
        ...
        fs::write(tmp.path().join("crane.toml"), "root").unwrap();
        let mid = tmp.path().join("sub0").join("sub1");
        fs::write(mid.join("crane.toml"), "mid").unwrap();
