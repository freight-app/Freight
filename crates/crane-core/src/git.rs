...
//!
//! Using libgit2 keeps crane self-contained — no `git` binary required on
...
pub fn clone_dep(
    ...
    if let Some(sha) = rev {
        ...
        let obj = repo
            .revparse_single(sha)
            .map_err(|e| CraneError::GitError(format!("rev `{sha}` not found: {e}")))?;
        repo.checkout_tree(&obj, None)
            .map_err(|e| CraneError::GitError(format!("checkout `{sha}`: {e}")))?;
        repo.set_head_detached(obj.id())
            .map_err(|e| CraneError::GitError(e.to_string()))?;
    } else {
...
pub fn update_dep(
    ...
    let repo = Repository::open(dest)
        .map_err(|e| CraneError::GitError(format!("open repo at {}: {e}", dest.display())))?;
    ...
    let remote_ref = if let Some(b) = branch.or(tag) {
        ...
        let head = repo
            .head()
            .map_err(|e| CraneError::GitError(format!("HEAD: {e}")))?;
        head.shorthand()
    ...
    let mut remote = repo
        .find_remote("origin")
        .map_err(|e| CraneError::GitError(format!("remote origin: {e}")))?;
    ...
    let fetch_head = repo
        ...
        .map_err(|e| CraneError::GitError(format!("FETCH_HEAD: {e}")))?;
    let obj = fetch_head
        .peel(git2::ObjectType::Commit)
        .map_err(|e| CraneError::GitError(e.to_string()))?;
    repo.reset(&obj, ResetType::Hard, None)
        .map_err(|e| CraneError::GitError(format!("reset: {e}")))?;
...
fn with_auth<T, F>(f: F) -> Result<T, CraneError>
where
    F: FnOnce(FetchOptions<'_>) -> Result<T, git2::Error>,
{
    ...
    f(fo).map_err(|e| CraneError::GitError(e.to_string()))
}
