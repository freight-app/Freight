...
/// Persistent cache of detected compiler versions, stored at `~/.crane/toolchain-cache.json`.
///
...
fn cache_path() -> Option<PathBuf> {
    crane_home().map(|h| h.join("toolchain-cache.json"))
}

/// The crane home directory: `$CRANE_HOME` or `~/.crane`.
pub fn crane_home() -> Option<PathBuf> {
    ...
    Some(PathBuf::from(home).join(".crane"))
}
