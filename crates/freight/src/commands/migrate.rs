use freight_migrator::{parse_format, run_migrate};
        ...
        Ok(outcome) => {
            if outcome.is_workspace {
                if outcome.written_to.is_none() {
                    ...
                    println!("# === workspace root freight.toml ===");
                    print!("{}", outcome.toml);
                ...
                if outcome.note_count > 0 {
                    print_warning(&format!(
                        "{} construct(s) could not be imported — see `# FREIGHT:` comments",
                        outcome.note_count,
                    ));
                }
                ...
                for (dir, _) in &outcome.workspace_members {
                    let member_path = cwd.join(dir).join("freight.toml");
            ...
            if outcome.note_count > 0 {
                print_warning(&format!(
                    "{} construct(s) could not be imported — see `# FREIGHT:` comments in freight.toml",
                    outcome.note_count,
                ));
            }
