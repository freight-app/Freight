...
everything above applied. The configure also defaults
`-DCMAKE_POLICY_VERSION_MINIMUM=3.5` (overridable via `[package].defines`), since
CMake 4.x hard-rejects the `cmake_minimum_required(VERSION <3.5)` that most pre-2020
libraries still declare, and the foreign sub-build's parallelism is bounded to
