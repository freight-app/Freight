...
- a **freight package** — a `{ path = "..." }` dependency in the parent manifest, or
  one fetched into `.pkgs/<name>/` — built natively and wrapped in a generated `.pc`
  + `<Name>Config.cmake` (see export, below). The `path` case lets a foreign CMake
...
everything above applied. The cmake plugin probes `cmake --version` and, on
CMake ≥ 4.0 only, defaults `-DCMAKE_POLICY_VERSION_MINIMUM=3.5` (overridable via
`[package].defines`), since CMake 4.0 removed the `cmake_minimum_required(VERSION
<3.5)` that most pre-2020 libraries still declare; CMake 3.x still honours those
minimums and is left untouched. The foreign sub-build's parallelism is bounded to
freight's `--jobs` (the cmake/meson/autotools plugins pass `JOBS` through). Such a manifest can be written by hand or generated by the
